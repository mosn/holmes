...
func (h *Holmes) writeString(content string) {
	logger, ok := h.Logger.Load().(*os.File)
	if !ok || logger == nil {
		//nolint
	...
	if _, err := h.Logger.Load().(*os.File).WriteString(content); err != nil {
		//nolint
		fmt.Println(err) // where to write this log?
		return
	}

	if !h.logOpts.RotateEnable {
		return
	}
	...
	if state.Size() > h.logOpts.SplitLoggerSize && atomic.CompareAndSwapInt32(&h.changelog, 0, 1) {
